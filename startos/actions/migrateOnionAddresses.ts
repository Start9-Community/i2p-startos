...
const inputSpec = InputSpec.of({
  addresses: Value.dynamicMultiselect(async ({ effects }) => {
    ...
    for (let i = 0; i < entries.length; i++) {
      ...
      // Skip keys that aren't properly clamped — they were generated by
      // a past bug and would derive a different hostname than Tor used
      if (!entry.key) continue
      const keyBytes = Buffer.from(entry.key, 'base64')
      if (keyBytes.length < 64 || !isClamped(keyBytes.subarray(0, 32)))
        continue
